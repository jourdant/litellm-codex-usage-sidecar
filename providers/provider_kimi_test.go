...
func TestKimiProviderMapsQuotaWindows(t *testing.T) {
	...
	usage := fetchKimi(t, upstream, upstream.URL)
...
func TestKimiProviderParsesCodingUsagesEndpoint(t *testing.T) {
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Header.Get("Authorization") != "Bearer kimi-token" || r.Header.Get("x-api-key") != "kimi-token" {
	...
	usage := fetchKimi(t, upstream, upstream.URL+"/coding/v1/usages")
	if usage.Primary == nil || usage.Secondary == nil || usage.Primary.UsedPercent != 89 || usage.Secondary.UsedPercent != 36 {
		t.Fatalf("unexpected usage windows: %+v", usage)
	}
}

func TestKimiProviderDerivesMissingWeeklyUsedValue(t *testing.T) {
	t.Parallel()

	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"usage":{"limit":"100","remaining":"85","resetTime":"2026-07-24T10:37:36.061466Z"},"limits":[{"window":{"duration":300,"timeUnit":"TIME_UNIT_MINUTE"},"detail":{"limit":"100","used":"2","remaining":"98","resetTime":"2026-07-17T15:37:36.061466Z"}}]}`))
	}))
	defer upstream.Close()

	usage := fetchKimi(t, upstream, upstream.URL+"/coding/v1/usages")
	if usage.Secondary == nil || usage.Secondary.UsedPercent != 15 || usage.Secondary.RemainingPercent != 85 {
		t.Fatalf("unexpected derived weekly usage: %+v", usage)
	}
}

func fetchKimi(t *testing.T, upstream *httptest.Server, usageURL string) UsageResponse {
	t.Helper()
	authPath := filepath.Join(t.TempDir(), "kimi-auth.json")
	if err := os.WriteFile(authPath, []byte(`{"access_token":"kimi-token"}`), 0o600); err != nil {
		t.Fatal(err)
	}
	usage, err := Fetch(context.Background(), upstream.Client(), Plan{Provider: "kimi", Plan: "kimi_code_plan_01", UsageURL: usageURL, AuthFile: authPath})
	if err != nil {
		t.Fatal(err)
	}
	return usage
